...
category: Ruby, Roda
image: /posts/ruby-on-roda-api/cover.png
---

<span style={{ color: '#9b111e', letterSpacing: '0.09em', fontSize: '30px', fontWeight: '600' }}>Roda</span> is a lightweight and productive framework for building web applications using <span style={{ color: '#9b111e', letterSpacing: '0.09em', fontSize: '15px', fontWeight: '600' }}>Ruby</span>. As mentioned in Roda’s [Website](https://roda.jeremyevans.net/), Roda is designed to be simple, both internally and externally, reducing cognitive overhead. Roda uses a routing tree which means at any point during routing, it allows you to operate on the current request.

![First one](/posts/ruby-on-roda-api/1.png)

Let’s stop praising Roda and start building some useful things.


Before you go through this article, you need to have some basic knowledge about <span style={{ color: '#9b111e', letterSpacing: '0.09em', fontSize: '15px', fontWeight: '600' }}>Ruby</span> -> [<span style={{ color: '#9b111e', letterSpacing: '0.09em', fontSize: '15px', fontWeight: '600' }}>Get started with Ruby</span>](https://www.ruby-lang.org/en/)
...
![First one](/posts/ruby-on-roda-api/2.png)
...
![First one](/posts/ruby-on-roda-api/3.png)
...
The main file in the app is <code>app.rb</code> and this file has been loaded via <code>config.ru</code> to serve the application with <code>rackup</code> command
...
Run <code>rackup</code> now, make GET request to our app’s root / URL, and you should see server status response.
...
![First one](/posts/ruby-on-roda-api/4.png)
...
Create the models files for <code>category, item, and order</code>
...
let’s create migration files for those models as well <code>category, item, and order</code>
...
Here we go!!!! run <code>rake db:migrate</code>
...
let’s create the controller’s files for <code>categories, items, and orders</code>
